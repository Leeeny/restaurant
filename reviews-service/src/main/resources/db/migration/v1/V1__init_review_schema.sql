...
CREATE UNIQUE INDEX menu_id_created_by_idx ON review.reviews (menu_id, created_by);

CREATE TABLE review.ratings
(
    id           BIGINT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
    menu_id      BIGINT  NOT NULL UNIQUE,
    rate_one     INTEGER NOT NULL DEFAULT 0, -- количество оценок 1
    rate_two     INTEGER NOT NULL DEFAULT 0, -- количество оценок 1
...
CREATE OR REPLACE FUNCTION review.calculate_wilson_score()
    RETURNS TRIGGER AS
$$ -- функцию можно вызвать только через триггер
DECLARE -- локальные переменные функции
    positive        FLOAT; -- взвешенное количество положительных голосов, у наивысшей оценки коэффициент 1, у низшей 0
    negative        FLOAT; -- аналогично
    prevWilsonScore FLOAT; -- предыдущее значение wilsonScore
    wilsonScore     FLOAT; -- текущее
    total           INTEGER; -- сколько всего оценок поставлено
BEGIN
    ...
    FROM review.ratings
    WHERE menu_id = NEW.menu_id; -- NEW это специальная запись, содержащая новую версию строки, вызвавшей триггер
...
CREATE TRIGGER trigger_update_wilson_score
    ...
    on review.ratings
    FOR EACH ROW
EXECUTE FUNCTION review.calculate_wilson_score();